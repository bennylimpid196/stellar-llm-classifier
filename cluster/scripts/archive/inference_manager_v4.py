...
from system_prompt_v4 import SYSTEM_PROMPT, build_prompt, build_prompt_reduced, PROMPT_VERSION

# ---------------------------------------------------------------------------
# Logging — append mode preserves logs across SLURM runs
# ---------------------------------------------------------------------------
logging.basicConfig(
    filename="inference_manager_sc_v4.log",
    filemode="a",
    level=logging.INFO,
...
def _compile_global(output_dir: Path, job_id: str) -> None:
    ...
    for batch_file in sorted(
        output_dir.glob("batch_v4_*.json"),
        key=lambda p: int(p.stem.split("_")[2]),  # batch_v4_N_JID: index is third segment
    ):
        try:
    ...
    global_path = output_dir / f"sc_results_v4{suffix}.json"
...
def run_inference(
    ...
    for batch_file in output_dir.glob("batch_v4_*.json"):
        try:
            stem = batch_file.stem           # e.g. "batch_3_180656"
            idx  = int(stem.split("_")[2])   # batch_v4_N: index is third segment
    ...
    for i, batch in enumerate(batches):
        ...
        batch_path   = output_dir / f"batch_v4_{batch_idx}{suffix_batch}.json"
    ...
    if all_failed:
        suffix      = f"_{job_id}" if job_id else ""
        failed_path = output_dir / f"sc_failed_v4{suffix}.json"

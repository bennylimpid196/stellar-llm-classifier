...
from system_prompt_v5 import SYSTEM_PROMPT, build_prompt, build_prompt_reduced, PROMPT_VERSION

# ---------------------------------------------------------------------------
# Logging — append mode preserves logs across SLURM runs
# ---------------------------------------------------------------------------
logging.basicConfig(
    filename="inference_manager_sc_v5.log",
    filemode="a",
    level=logging.INFO,
...
_REQUIRED_TOP = {"source_id", "classification", "confidence_scores", "technical_reasoning", "stellar_description"}
...
def validate_sc_output(result: dict, source_id: str) -> list[str]:
    ...
    # stellar_description validation
    sd = result.get("stellar_description", {})
    for field in ("physical_profile", "population_context", "notable_features"):
        if field not in sd or not sd[field]:
            errors.append(f"stellar_description.{field} missing or empty.")
...
def _compile_global(output_dir: Path, job_id: str) -> None:
    ...
    for batch_file in sorted(
        output_dir.glob("batch_v5_*.json"),
        key=lambda p: int(p.stem.split("_")[2]),  # batch_v5_N_JID: index is third segment
    ):
        try:
    ...
    global_path = output_dir / f"sc_results_v5{suffix}.json"
...
def run_inference(
    ...
    for batch_file in output_dir.glob("batch_v5_*.json"):
        try:
            stem = batch_file.stem           # e.g. "batch_3_180656"
            idx  = int(stem.split("_")[2])   # batch_v5_N: index is third segment
    ...
    for i, batch in enumerate(batches):
        ...
        batch_path   = output_dir / f"batch_v5_{batch_idx}{suffix_batch}.json"
    ...
    if all_failed:
        suffix      = f"_{job_id}" if job_id else ""
        failed_path = output_dir / f"sc_failed_v5{suffix}.json"

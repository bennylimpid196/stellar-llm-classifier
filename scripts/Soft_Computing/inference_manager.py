...
from system_prompt import SYSTEM_PROMPT, build_prompt, build_prompt_reduced

# ---------------------------------------------------------------------------
# Logging — append mode preserves logs across SLURM runs
# ---------------------------------------------------------------------------
logging.basicConfig(
    filename="inference_manager_sc.log",
    filemode="a",
    level=logging.INFO,
...
def _compile_global(output_dir: Path, job_id: str) -> None:
    ...
    global_path = output_dir / f"sc_results{suffix}.json"
...
def run_inference(
    ...
    log.info("=" * 60)
    log.info(f"SC Stellar Inference Engine v1.0 — job_id={job_id or 'N/A'}")
    log.info("=" * 60)
    ...
    for batch_file in output_dir.glob("batch_*.json"):
        try:
            ...
            idx  = int(stem.split("_")[1])   # always the second segment
    ...
    for i, batch in enumerate(batches):
        ...
        batch_path   = output_dir / f"batch_{batch_idx}{suffix_batch}.json"
    ...
    if all_failed:
        suffix      = f"_{job_id}" if job_id else ""
        failed_path = output_dir / f"sc_failed{suffix}.json"
